...
8. after upgrading the package later, run `next-codex-workflow update`
...
There are now two different refresh paths, and they serve different purposes.

Use `update` when you already installed this workflow before and you want to
refresh the generated files after upgrading the package:
...
- `update` is the normal way to apply newer package output to an existing repo
- `init --overwrite-managed` is the reconfiguration path
- both still protect user-owned handwritten files
